...
import com.github.SnowFlakes.unit.InterAction;
...
public class MatrixItem extends AbstractItem {
    ...
    public BufferedImage DrawHeatMap(ArrayList<ChrRegion> bin_size, int resolution, float thresholdValue) {
        ...
        for (int i = 0; i < bin_size.size(); i++) {
            ...
            if (i > 0) {
                g.drawLine(marginal + r.region.Start * fold - 1, marginal, marginal + r.region.Start * fold - 1,
                        marginal + High * fold);// draw vertical line
                g.drawLine(marginal, (High - r.region.Start) * fold + marginal, marginal + Width * fold,
                        (High - r.region.Start) * fold + marginal);// draw horizontal line
            }
            Tools.DrawStringCenter(g, r.Chr, t, marginal + r.region.Center() * fold, High * fold + interval + marginal,
                    0);// draw X label
            Tools.DrawStringCenter(g, r.Chr, t, marginal - interval, (High - r.region.Center()) * fold + marginal,
                    -Math.PI / 2);// draw Y label
        }
        return image;
    }
    ...
    public BufferedImage DrawHeatMap(String Chr1, int StartSite1, String Chr2, int StartSite2, int resolution,
        ...
        BufferedImage image = new BufferedImage(MatrixWidth + Marginal * 2, MatrixHeight + Marginal * 2,
        ...
        if (Label) {
            ...
            for (int i = 0; i <= MatrixWidth * 10 / interval; i++) {
                graphics.drawLine(Marginal + i * interval / 10, Marginal + MatrixHeight, Marginal + i * interval / 10,
                        Marginal + MatrixHeight + extend_len / 2);
            }
            // draw x interval
            for (int i = 0; i <= MatrixWidth / interval; i++) {
                graphics.drawLine(Marginal + i * interval, Marginal + MatrixHeight, Marginal + i * interval,
                        Marginal + MatrixHeight + extend_len);
                float value = 0;
                switch (unit) {
                    case BP:
                ...
                Tools.DrawStringCenter(graphics, value_str, t, Marginal + i * interval,
                        Marginal + MatrixHeight + extend_len + h / 2 + 2, 0);
            }
            // draw sub y interval
        ...
        if (Legend) {
            interval = 40;
            t = new Font("Times New Roman", Font.PLAIN, 30);
            graphics.setPaint(new GradientPaint(Marginal + MatrixWidth + interval, Marginal + MatrixHeight, Color.WHITE,
                    Marginal + MatrixWidth + interval, Marginal, Color.RED));
            graphics.fillRect(Marginal + MatrixWidth + interval, Marginal, LegendWidth, MatrixHeight);
            graphics.setColor(Color.BLACK);
            for (int i = 0; i <= 10; i++) {
                graphics.drawLine(Marginal + MatrixWidth + interval + LegendWidth,
                        Math.round(Marginal + MatrixHeight - (float) (i) / 10 * MatrixHeight),
                        Marginal + MatrixWidth + interval + LegendWidth + extend_len,
                        Math.round(Marginal + MatrixHeight - (float) (i) / 10 * MatrixHeight));
                String value_str = String.format("%.1f", MinValue + (ThresholdValue - MinValue) * (float) (i) / 10);
                Tools.DrawStringCenter(graphics, value_str, t,
                        Marginal + MatrixWidth + interval + LegendWidth + extend_len + 2
                                + FontDesignMetrics.getMetrics(t).stringWidth(value_str) / 2,
                        Math.round(Marginal + MatrixHeight - (float) (i) / 10 * MatrixHeight), 0);
            }
        }
        // draw x,y title
        if (Label) {
            t = new Font("Times New Roman", Font.BOLD, 80);
            Tools.DrawStringCenter(graphics, Chr1, t, FontDesignMetrics.getMetrics(t).getHeight() / 2 + 5,
